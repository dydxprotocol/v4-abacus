...
class V4StateManagerAdaptor(
    ...
    private val chainHelper = ChainHelper(parser, environment.environment)
    ...
    private val pollingDuration = 10.0
    ...
    private fun didSetValidatorConnected(validatorConnected: Boolean) {
        if (validatorConnected) {
            ...
            heightTimer = timer.schedule(0.0, pollingDuration) {
                if (readyToConnect) {
    ...
    private fun getOnChain(
        ...
        if (query === null) {
            throw Exception("chain query ires null")
        }
