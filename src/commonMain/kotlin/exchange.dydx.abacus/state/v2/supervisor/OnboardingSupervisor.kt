...
internal class OnboardingSupervisor(
    ...
    private fun retrieveTransferChains() {
        ...
        if (url != null && squidIntegratorId != null) {
            ...
            helper.get(url, null, header) { _, response, httpCode ->
            ...
            helper.get(url, params, header) { _, response, code ->
                if (response != null) {
                    ...
                    if (currentFromAmount == oldFromAmount) {
                        update(stateMachine.squidRoute(response, subaccountNumber ?: 0), oldState)
                    }
                } else {
    ...
    private fun retrieveDepositRouteV2(state: PerpetualState?, subaccountNumber: Int?) {
        ...
        if (fromChain != null &&
            ...
            helper.post(url, header, body.toJsonPrettyPrint()) { _, response, code ->
                if (response != null) {
                    val currentFromAmount = stateMachine.state?.input?.transfer?.size?.size
                    val oldFromAmount = oldState?.input?.transfer?.size?.size
                    if (currentFromAmount == oldFromAmount) {
                        update(stateMachine.squidRouteV2(response, subaccountNumber ?: 0), oldState)
                    }
                } else {
    ...
    private fun retrieveWithdrawalRouteNoble(
        ...
        if (nobleChain != null &&
            ...
            helper.get(url, params, header) { _, response, _ ->
                if (response != null) {
                    update(stateMachine.squidRoute(response, subaccountNumber ?: 0), oldState)
                }
            }
    ...
    private fun retrieveWithdrawalRouteV2(
        ...
        if (toChain != null &&
            ...
            helper.post(url, header, body.toJsonPrettyPrint()) { url, response, code ->
                if (response != null) {
                    val currentFromAmount = stateMachine.state?.input?.transfer?.size?.size
                    val oldFromAmount = oldState?.input?.transfer?.size?.size
                    if (currentFromAmount == oldFromAmount) {
    ...
    private fun fetchTransferStatus(
        ...
        if (url != null && squidIntegratorId != null) {
            ...
            helper.get(url, params, header) { _, response, httpCode ->
                if (response != null) {
